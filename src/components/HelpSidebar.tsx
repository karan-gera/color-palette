    ...
    >
      ...
      {HELP_SECTIONS.map((section, sectionIndex) => (
        <section key={section} className="mb-5" aria-labelledby={`help-section-${sectionIndex}`}>
          <h2 id={`help-section-${sectionIndex}`} className="mb-1.5 text-[11px] font-normal tracking-wide text-[var(--docs-accent-text)]">
            {section}
          </h2>
            ...
            {HELP_CONTENT.filter((entry) => entry.section === section).map((entry) => (
              <li key={entry.id}>
                <button
                  ...
                  className={`w-full rounded-md px-2 py-1.5 text-left text-xs leading-5 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--docs-accent)] motion-reduce:transition-none ${
                    activePage === entry.id
                      ? 'bg-[var(--docs-accent-surface-strong)] font-medium text-[var(--docs-accent-text)]'
